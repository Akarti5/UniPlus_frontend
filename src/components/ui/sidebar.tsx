...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex items-center gap-2 data-[active=true]:bg-sidebar-accent data-[state=open]:hover:bg-sidebar-accent hover:bg-sidebar-accent active:bg-sidebar-accent aria-disabled:opacity-50 disabled:opacity-50 p-2 group-data-[collapsible=icon]:!p-2 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 rounded-md outline-none ring-sidebar-ring focus-visible:ring-2 w-full [&>svg]:size-4 group-data-[collapsible=icon]:!size-8 overflow-hidden font-semibold data-[active=true]:font-bold text-sm text-left [&>span:last-child]:truncate transition-[width,height,padding] data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:text-sidebar-accent-foreground hover:text-sidebar-accent-foreground active:text-sidebar-accent-foreground cursor-pointer disabled:cursor-not-allowed aria-disabled:pointer-events-none disabled:pointer-events-none [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<"ul">>(
  ({ className, ...props }, ref) => (
    <ul
      ...
      className={cn(
        "flex flex-col gap-0.5 mx-3.5 px-2.5 py-1 border-sidebar-border border-l-2 min-w-0 translate-x-px",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        // Base : plus petit, plus léger, et légèrement transparent
        "flex items-center gap-2 hover:bg-sidebar-accent active:bg-sidebar-accent aria-disabled:opacity-50 disabled:opacity-50 px-2 rounded-md outline-none ring-sidebar-ring focus-visible:ring-2 min-w-0 h-6 [&>svg]:size-3.5 overflow-hidden font-normal text-sidebar-foreground/65 [&>span:last-child]:truncate -translate-x-px [&>svg]:text-sidebar-accent-foreground hover:text-sidebar-accent-foreground active:text-sidebar-accent-foreground cursor-pointer disabled:cursor-not-allowed aria-disabled:pointer-events-none disabled:pointer-events-none [&>svg]:shrink-0",
        // État actif : couleur pleine + léger fond
        "data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[active=true]:font-medium",
        // Taille de police selon la prop size
        size === "sm" && "text-xs",
        size === "md" && "text-xs",
        "group-data-[collapsible=icon]:hidden",
        className,
